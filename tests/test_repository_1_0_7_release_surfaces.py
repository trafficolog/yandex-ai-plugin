...
class Repository107ReleaseSurfaceTests(unittest.TestCase):
    ...
    def test_root_readmes_stage_repository_1_0_7(self):
        for filename in ("README.md", "README.en.md"):
            text = self.read(filename)
            with self.subTest(filename=filename):
                self.assertIn("release-1.0.7", text)
                self.assertIn("1.0.7", text)

    def test_bilingual_changelogs_stage_repository_1_0_7(self):
        marker = "## [1.0.7] — 2026-09-05"
        for filename in ("CHANGELOG.md", "CHANGELOG.en.md"):
            with self.subTest(filename=filename):
                self.assertIn(marker, self.read(filename))

    def test_declarative_manifest_is_repository_only_1_0_7(self):
        data = json.loads(self.read(".github/releases/release.json"))
        self.assertEqual(data["repository"]["version"], "1.0.7")
        self.assertEqual(data["repository"]["tag"], "1.0.7")
        self.assertEqual(data["repository"]["title"], "Repository 1.0.7")
        self.assertEqual(data["repository"]["notes_file"], ".github/releases/1.0.7.md")
        self.assertEqual(data["plugins"], [])
        self.assertTrue((ROOT / ".github/releases/1.0.7.md").is_file())
