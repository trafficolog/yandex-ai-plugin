...
Marketplace policy: `.agents` entry использует `authentication: ON_USE`; для transport-free SEO authentication выполняется только в owning service plugins. Полная семантика deferred authentication и transport ownership: [`docs/ARCHITECTURE.md`](../../docs/ARCHITECTURE.md).
