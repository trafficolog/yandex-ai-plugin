...
- Internal-link audit считает orphan по отсутствию inbound links, сохраняет и флагирует duplicate links, а rootless `BRIDGE` без inbound link отмечает как orphan/broken bridge; `ROOT` остаётся exempt.
