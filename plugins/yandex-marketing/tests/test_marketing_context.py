...
from scripts.marketing_context import (
    normalize_query, normalize_url, kpi_fingerprint,
    compare_kpi_fingerprints, classify_period_alignment, classify_maturity,
)
...
class MarketingContextTests(unittest.TestCase):
    ...
    def test_kpi_fingerprint_comparison_reports_material_mismatch(self):
        base = kpi_fingerprint({'business_objective':'purchase','goal_ids':['2','1'],'attribution_model':'automatic','metric_basis':'converted_sessions','currency':'RUB','vat_basis':'excluded','period':{'from':'2026-08-01','to':'2026-08-31'}})
        same = kpi_fingerprint({**base, 'goal_ids':['1','2']})
        self.assertTrue(compare_kpi_fingerprints(base, same)['compatible'])
        for key, value in [('goal_ids',['9']),('attribution_model','last'),('metric_basis','users'),('currency','EUR'),('vat_basis','included'),('period',{'from':'2026-07-01','to':'2026-07-31'})]:
            other = dict(base); other[key] = value
    ...
    def test_period_alignment_states(self):
        a={'period':{'from':'2026-08-01','to':'2026-08-31'}}
        b={'period':{'from':'2026-08-01','to':'2026-08-31'}}
        self.assertEqual(classify_period_alignment([a,b]), 'EXACT')
        c={'period':{'from':'2026-08-02','to':'2026-09-01'}, 'approximate_period': True}
        self.assertEqual(classify_period_alignment([a,c]), 'APPROXIMATE')
        d={'period':{'from':'2026-06-01','to':'2026-06-30'}}
        self.assertEqual(classify_period_alignment([a,d]), 'MISMATCHED')
