...
def reconcile_metric(metric: str, records: list[dict], context: dict) -> dict:
    ...
    if len(relevant) < 2:
        return {'metric': metric, 'status': 'REVIEW', 'records': relevant, 'reason': 'insufficient overlapping evidence'}
    kpis = [r.get('kpi') for r in relevant if isinstance(r.get('kpi'), dict)]
    if len(kpis) >= 2:
        baseline = kpis[0]
        for other in kpis[1:]:
            comparison = compare_kpi_fingerprints(baseline, other)
            if not comparison['compatible']:
                return {'metric': metric, 'status': 'INCOMPARABLE', 'records': relevant, 'mismatches': comparison['mismatches']}
    values = [r.get('value') for r in relevant]
    if all(value == values[0] for value in values[1:]):
        return {'metric': metric, 'status': 'ALIGNED', 'records': relevant}
    ...
    return {'metric': metric, 'status': 'REVIEW', 'records': relevant}
...
def propagate_limitations(source_records: list[dict]) -> list[dict]:
    ...
    for record in source_records:
        ...
        if record.get('sampled'):
            result.append({'code': 'METRIKA_SAMPLED', 'source': source, 'sample_share': record.get('sample_share')})
        if record.get('data_lag') is not None:
            result.append({'code': 'DATA_LAG', 'source': source, 'value': record.get('data_lag')})
