...
def new_bundle(context: dict, coverage: dict) -> dict:
    ...
    normalized_coverage = {name: bool(coverage.get(name, False)) for name in ('direct','metrika','wordstat','search')}
    return {
        'version': 1,
        'context': dict(context),
        'coverage': normalized_coverage,
        'routing_required': not normalized_coverage['direct'],
        'campaigns': [], 'criteria': [], 'search_queries': [], 'landings': [],
        'goals': [], 'demand': [], 'sources': {}, 'evidence': [], 'findings': [], 'limitations': [],
    }
