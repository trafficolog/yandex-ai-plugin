...
Never sum overlapping row counts and label them **total demand**, market size, or unique searches. Keep Yandex `totalCount` per individual seed/expression instead.
...
For large collections, store JSON/file output with filters, backend, timestamp, relation types and source seeds.
